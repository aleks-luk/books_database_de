...
def transform_users():
    users_frame = pd.read_csv('./raw_data/users.csv', usecols=['Location', 'Age'], delimiter=';', encoding='windows-1250', low_memory=False)
    ...
    users_frame.rename(columns=newUsersColumnNames, inplace=True)
...
def transform_ratings():
    ...
    ratings_frame.rename(columns=newRatingsColumnNames, inplace=True)
...
def transform_user_api_data():
    with open('./raw_data/users_api_data.json', 'r', encoding='utf-8') as f:
    ...
    df_users.rename(columns=newUsersApiDataColumnNames, inplace=True)
...
def transform_scrapped_books_data():
    ...
    scrapped_data.rename(columns=newScrappedDataColumnNames, inplace=True)
